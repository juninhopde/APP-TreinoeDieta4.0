...
const IA_PROVEDORES = {
  gemini: {
    ...
    async chamar({ chave, modelo, sistema, pergunta }) {
      ...
      const modeloLimpo = String(modelo || this.modeloPadrao).trim();

      const url = 'https://generativelanguage.googleapis.com/v1beta/models/'
        + encodeURIComponent(modeloLimpo)
      ...
      if (!r.ok) {
        ...
        throw erroIA(r.status, textoErro, 'gemini');
      }
...
function traduzirFalha(err) {
  ...
  if (/failed to fetch|networkerror|load failed|network request failed/i.test(m))
    return new Error('Sem ligação ao provedor. Verifique a sua internet ou bloqueios de rede.');
...
REGRAS ABSOLUTAS:
1. Usa APENAS os números do CONTEXTO abaixo.
2. Se faltarem dados, diz o que a pessoa precisa de registar.
3. Não prescrevas medicamentos.
4. Perante qualquer sinal de risco, orienta a procura de um profissional.
5. Para registar uma ação, propõe o JSON da ferramenta exato: {"ferramenta":"nome","argumentos":{...}}
